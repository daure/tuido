...
const ABOUT: &str =
    "Keyboard-first task manager with MCP access to tasks, people, spaces, and tags.
...
enum Commands {
    #[command(
        ...
        long_about = "Run tasks, people, spaces, and tags MCP server over stdio. stdout is reserved for JSON-RPC protocol output; diagnostics belong on stderr."
    )]
    Mcp,
    #[command(
